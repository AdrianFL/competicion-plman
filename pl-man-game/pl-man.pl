...
p_loadBehaviours_loadLanguage(BLIBPATH) :-
	showSystemMsg(system, error, error_loading_behaviour_language_file, BLIBPATH, [abort]).
...
%		), !.
p_loadBehaviours:-
	showSystemMsg(system, error, error_loading_behaviours, '', [abort]).
...
loadNewMap(File):-
	...
	loadNewMap(version, V), !.
loadNewMap(File):-
	showSystemMsg(system, error, map_file_error, File, [abort]).
...
getSeeList(row, X, Y, Row, Add, [H|T]):-
        not((entityLocation(_, X, Y, EE), EE\='')),
	nth0(X, Row, H),
        not(isSolid(H)),
	X1 is X + Add,
	getSeeList(row, X1, Y, Row, Add, T).
