...
initMap:- 
	...
	respawnPoint(init, [OID_RS0], 2, 
		...
		[ EID_RP0, 'pl-man':entitySequentialMovement(init, EID_RP0, RP0_MOVEMENT, [no_repeat_moves]) ], [ probRespawn(200) ]),

	respawnPoint(init, [OID_RS1], 1, 
		[ app('F'), name(fantasma), crule(spiderGhost), appearance(attribs(normal, yellow, default)) ],
		[ EID_RP1, 'pl-man':spiderGhost(init, EID_RP1, ['#'], [ probFollowPacman(100), maxFollowTimes(1) ] ) ], [ probRespawn(100) ]),

	respawnPoint(init, [OID_RS2], 2, 
		[ app('E'), name(enemigo), crule(entitySequentialMovement), appearance(attribs(normal, magenta, default)) ],
		[ EID_RP2, 'pl-man':entitySequentialMovement(init, EID_RP2, RP2_MOVEMENT, [no_repeat_moves]) ], [ probRespawn(200) ]),
